...
def render_frontiers_comparison(grid, 
    ...
    for idx, cluster in enumerate(clusters_split):
        if len(cluster) > 0:
            color = colors[idx % len(colors)]
            ax_split.scatter(cluster[:, 0], cluster[:, 1], c=[color], s=200, 
                            alpha=0.7, edgecolors='black', linewidth=0.5, 
                            marker='o', zorder=3)
...
def render_frontiers_comparison_with_bboxes(grid, 
                                             centroids_orig, 
                                             clusters_orig, 
                                             centroids_split, 
                                             clusters_split, 
    ...
    """
    Render two frontier cluster configurations side-by-side with professional thesis-quality visualization.
    Draws bounding boxes around clusters that will be split.
    ...
    max_cluster_size : int
        Threshold size for splitting clusters
    """
    ...
    # Plot clusters and draw bounding boxes around clusters that will be split
    for idx, cluster in enumerate(clusters_orig):
        if len(cluster) > 0:
            color = colors[idx % len(colors)]
            ax_orig.scatter(cluster[:, 0], cluster[:, 1], c=[color], s=200, 
                           alpha=0.7, edgecolors='black', linewidth=0.5, 
                           marker='o', zorder=3)
            
            # Draw bounding box if cluster will be split
            if len(cluster) > max_cluster_size:
                x_min, y_min = cluster.min(axis=0)
                x_max, y_max = cluster.max(axis=0)
                width_box = x_max - x_min + 1
                height_box = y_max - y_min + 1
                
                rect = Rectangle((x_min - 0.5, y_min - 0.5), width_box, height_box, 
                                linewidth=2.5, edgecolor='red', facecolor='none', 
                                linestyle='--', zorder=5, label='Split axis' if idx == 0 else '')
                ax_orig.add_patch(rect)
                
                # Draw arrow lines from center towards box edges
                x_center = (x_min + x_max) / 2
                y_center = (y_min + y_max) / 2
                arrow_length = max(width_box, height_box) / 2 - 0.4
                
                # Horizontal arrows (left and right)
                arrow_right = FancyArrowPatch((x_center, y_center), 
                                            (x_center + arrow_length, y_center),
                                            arrowstyle='->', mutation_scale=20, 
                                            linewidth=2.5, color='red', zorder=4)
                arrow_left = FancyArrowPatch((x_center, y_center), 
                                           (x_center - arrow_length, y_center),
                                           arrowstyle='->', mutation_scale=20, 
                                           linewidth=2.5, color='red', zorder=4)
                
                # Vertical arrows (up and down)
                arrow_down = FancyArrowPatch((x_center, y_center), 
                                           (x_center, y_center + arrow_length),
                                           arrowstyle='->', mutation_scale=20, 
                                           linewidth=2.5, color='red', zorder=4)
                arrow_up = FancyArrowPatch((x_center, y_center), 
                                         (x_center, y_center - arrow_length),
                                         arrowstyle='->', mutation_scale=20, 
                                         linewidth=2.5, color='red', zorder=4)
                
                ax_orig.add_patch(arrow_left)
                ax_orig.add_patch(arrow_right)
                ax_orig.add_patch(arrow_up)
                ax_orig.add_patch(arrow_down)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#06A77D', 
               markersize=8, markeredgecolor='black', markeredgewidth=1.5, label='Agent'),
        Line2D([0], [0], color='red', linewidth=2.5, linestyle='--', label=f'Split axis'),
        Patch(facecolor='white', edgecolor='black', linewidth=1.5, label='Free cells'),
        Patch(facecolor='lightgray', edgecolor='gray', linewidth=0.5, label='Unknown cells'),
        Patch(facecolor='black', edgecolor='gray', linewidth=0.5, label='Occupied cells')
    ]
