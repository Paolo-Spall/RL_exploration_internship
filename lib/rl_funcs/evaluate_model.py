...
def evaluate_model_old(model_name, check=False, dir="", notrunc_flag=False):
    if dir:
        dir = dir.strip('/') +'/'
...
def evaluate_model(model_filepath, config_filepath, check=False, dir=""):
    if dir:
        dir = dir.strip('/') +'/'
    ...
    output_file = f"{dir}evaluation_{model_filename}.txt"
    with open(output_file, "a") as f:
        f.write(f"Mean reward = {mean_reward:.2f} +/- {std_reward:.2f}\n")
        f.write(f"Action statistics:\n")
        f.write(f"  Mean action: {action_stats['mean_action']:.4f}\n")
